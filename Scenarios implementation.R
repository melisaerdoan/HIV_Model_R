...
# IMPORTANT :lines between 109 and 112 should be manually determined. 
library(deSolve)
library(MASS)
library(mvtnorm)
...
run_model <- function(scenario_number, scenario_change_in_VCT, scenario_change_in_others) {   
  ...
  params1 <- c( beta1 = beta_1, beta2 = beta_2, sigma1 = 0.00206, sigma2 = 0.13241, delta1 = 0.00434, delta2 = 0.19086, epsilon1 = 0.53
               , epsilon2 = 0.96,  omega = 2, psi = psi_param,  alpha = 0.747)
...
scenario_count <- 5 
main_scenario <- 3
scenario_change_in_VCT <- c(1, 2, 4, 1, 1)           # 5-2 c(1, 2, 4, 2.6, 3.8)  # 5-3 c(1, 2, 4, 1, 1)        #3-4 c(1,2,4)
scenario_change_in_others <- c(1, 1, 1, 2, 4)               # 5-2 c(1, 1, 1, 1, 1)        # 5-3 c(1, 1, 1, 2, 4)  # 3-4 c(1,2,4)
scenario_results <- list()
prevented_incidence_per_increased_test_VCT <- c()
prevented_incidence_per_increased_test_others <- c()
all_results <- c()
col <- c( "red", "blue", "orange","limegreen", "gray15" , "brown", "navyblue")
...
par(mfrow = c(1,1))
years <- 2018:2033
CI_intervals  <- read.csv("credible_intervals.csv")
data_columns <- 5:20
...
for (s in 2:length(result_name_list)) {     # s starts from 2 because there is time in the first column and SH starts from the second column
  ...
  if (s == 12) {   # The y axis label for diagnosed cases is different
    ...
    # add shaded ribbon as credible intervals for diagnosed cases
    data_lower <- CI_intervals[CI_intervals $main_scenario == 1 & CI_intervals$scenario_count == 1 & CI_intervals$CI_type == "lower" & CI_intervals$result_type == "diagnosed", data_columns]
    data_upper <- CI_intervals[CI_intervals $main_scenario == 1 & CI_intervals$scenario_count == 1 & CI_intervals$CI_type == "upper"& CI_intervals$result_type == "diagnosed", data_columns]
    polygon(c(c((1+2017):(ending+1+2017)), rev(c((1+2017):(ending+1+2017)))), c(data_lower , rev(data_upper)),
            col = "gray80", border = NA)

    lines(c((1+2017):(ending+1+2017)), data_for_base_plot[[result_name_list[[s]]]], 
          col = col[1])
    ...
    plot(c((1+2017):(ending+1+2017)), data_for_base_plot[[result_name_list[[s]]]], 
          ...
          xaxt='n', col= col[1], las=2 )
    # add shaded ribbon as credible intervals for incidence and prevelance
    if (s == 10) {
      data_lower <- CI_intervals[CI_intervals $main_scenario == 1 & CI_intervals$scenario_count == 1 & CI_intervals$CI_type == "lower" & CI_intervals$result_type == "incidence", data_columns]
      data_upper <- CI_intervals[CI_intervals $main_scenario == 1 & CI_intervals$scenario_count == 1 & CI_intervals$CI_type == "upper"& CI_intervals$result_type == "incidence", data_columns]
    ...
    if (s == 11) {
      data_lower <- CI_intervals[CI_intervals $main_scenario == 1 & CI_intervals$scenario_count == 1 & CI_intervals$CI_type == "lower" & CI_intervals$result_type == "prevalence", data_columns]
      data_upper <- CI_intervals[CI_intervals $main_scenario == 1 & CI_intervals$scenario_count == 1 & CI_intervals$CI_type == "upper"& CI_intervals$result_type == "prevalence", data_columns]
      polygon(c(c((1+2017):(ending+1+2017)), rev(c((1+2017):(ending+1+2017)))), c(data_lower, rev(data_upper)),
              col = "gray80", border = NA)
    }
  ...
  # plot for scenarios other than base is created (with fold_list) 
      for (i in (2:scenario_count)){ # shaded ribbons in the graph
        
        if (s == 10) {
          data_lower <- CI_intervals[CI_intervals $main_scenario == main_scenario & CI_intervals$scenario_count == i & CI_intervals$CI_type == "lower" & CI_intervals$result_type == "incidence", data_columns]
          data_upper <- CI_intervals[CI_intervals $main_scenario == main_scenario & CI_intervals$scenario_count == i & CI_intervals$CI_type == "upper"& CI_intervals$result_type == "incidence", data_columns]
          polygon(c(c((1+2017):(ending+1+2017)), rev(c((1+2017):(ending+1+2017)))), c(data_lower , rev(data_upper)),
                  col = "gray80", border = NA)
        }
        
        if (s == 11) {
          data_lower <- CI_intervals[CI_intervals $main_scenario == main_scenario & CI_intervals$scenario_count == i & CI_intervals$CI_type == "lower" & CI_intervals$result_type == "prevalence", data_columns]
          data_upper <- CI_intervals[CI_intervals $main_scenario == main_scenario & CI_intervals$scenario_count == i & CI_intervals$CI_type == "upper"& CI_intervals$result_type == "prevalence", data_columns]
          polygon(c(c((1+2017):(ending+1+2017)), rev(c((1+2017):(ending+1+2017)))), c(data_lower , rev(data_upper)),
                  col = "gray80", border = NA)
        }
        
        if (s == 12) {
        data_lower <- CI_intervals[CI_intervals $main_scenario == main_scenario & CI_intervals$scenario_count == i & CI_intervals$CI_type == "lower" & CI_intervals$result_type == "diagnosed", data_columns]
        data_upper <- CI_intervals[CI_intervals $main_scenario == main_scenario & CI_intervals$scenario_count == i & CI_intervals$CI_type == "upper"& CI_intervals$result_type == "diagnosed", data_columns]
        polygon(c(c((1+2017):(ending+1+2017)), rev(c((1+2017):(ending+1+2017)))), c(data_lower , rev(data_upper)),
                col = "gray80", border = NA)
        }
